...
export default function EditRoleModal({ role, close, reload }: Props) {
  ...
  const save = async () => {
    ...
    try {
    await api.put(`/roles/${role.id}`, { name, description });
      ...
      <div className="w-full max-w-md rounded-xl bg-slate-900 border border-slate-700 p-5">
        ...
        <div className="flex justify-end space-x-2 pt-2">
          ...
          <button
            onClick={save}
            disabled={saving}
            className="px-4 py-2 text-xs rounded-md bg-yellow-500 hover:bg-yellow-600 text-white disabled:opacity-60"
          >
            {saving ? <Spinner /> : "Save"}
          </button>
        </div>
